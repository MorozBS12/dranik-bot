...
async function getSkinUrl(nickname) {
  try {
    ...
    if (data.success) {
      ...
      return `https://visage.surgeplay.com/full/512/${uuid}`;
    }
  } catch {}
  return `https://visage.surgeplay.com/full/512/${nickname}`;
}
...
async function generateResultCard({ nickname, oldTier, newTier, testerName, testerAvatarUrl, skinUrl }) {
  ...
  // ===== SKIN BOTTOM LEFT - верхня половина =====
  if (skinImg) {
    // Малюємо тільки верхню половину скіна (голова + тіло)
    const skinH = 480;
    const skinW = skinImg.width * (skinH / skinImg.height);
    // Кліп щоб обрізати нижню частину
    ctx.save();
    ctx.beginPath();
    ctx.rect(0, 0, skinW + 30, H);
    ctx.clip();
    // Водяний знак
    ctx.globalAlpha = 0.06;
    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 90px Noto';
    ctx.fillText(newTier, 40, 420);
    ctx.globalAlpha = 1;
    // Малюємо скін так щоб низ виходив за межі картинки
    ctx.drawImage(skinImg, 10, H - skinH + 80, skinW, skinH);
    ctx.restore();
  }
