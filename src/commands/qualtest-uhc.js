...
const CATEGORY_ID = '1487425707209654352';

module.exports = {
  data: new SlashCommandBuilder()
    .setName('qualtest-uhc')
    .setDescription('Квалификационный тест (UHC)'),
  ...
  async execute(interaction) {
    ...
    if (!hasPermission) {
      return interaction.reply({ content: '❌ У вас нет прав для вызова этой панели!', ephemeral: true });
    }

    await interaction.deferReply({ ephemeral: true });

    const embed = new EmbedBuilder()
      .setTitle('◈ Квалификационный тест — UHC')
      .setDescription('Нажмите кнопку ниже, чтобы пройти квалификационный тест на UHC.\n\n⏳ КД на тест — **7 дней**.\n⚠️ Предоставление недостоверной информации приведет к отказу.')
      .setFooter({ text: 'Система квалификации UHC' })
      .setTimestamp()
      .setColor(0xe67e22);

    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('btn_qualtest_uhc').setLabel('Пройти тест UHC').setStyle(ButtonStyle.Primary).setEmoji('📋'),
    );

    await interaction.channel.send({ embeds: [embed], components: [row] });
    await interaction.editReply({ content: '✅ Панель квалификации UHC отправлена!' });
  },

  async handleButton(interaction) {
    if (interaction.customId === 'btn_qualtest_uhc') {
      const modal = new ModalBuilder().setCustomId('qualtest_uhc_modal').setTitle('Квалификационный тест UHC');
      modal.addComponents(
  ...
  async handleModal(interaction) {
    if (interaction.customId !== 'qualtest_uhc_modal') return;
    ...
    const embed = new EmbedBuilder()
      .setTitle('📋 Заявка на квалификационный тест UHC')
      .addFields(
    ...
    await ticketChannel.send({ content: `<@&${MAIN_PING_ROLE}> | Новая заявка на тест UHC от ${user}`, embeds: [embed], components: [closeRow] });
    await interaction.reply({ content: `✅ Заявка на UHC отправлена! Канал: ${ticketChannel}`, ephemeral: true });
  },
};
