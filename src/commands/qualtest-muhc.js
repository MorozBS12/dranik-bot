...
const MODER_ROLES = [
  ...
  '1491116049792241674','1480616685999226971','1487138476133449870'
];
const MAIN_PING_ROLE = '1491116049792241674';
const CATEGORY_ID = '1487425707209654352';

module.exports = {
  data: new SlashCommandBuilder()
    .setName('qualtest-muhc')
    .setDescription('Квалификационный тест (mUHC)'),
  ...
  async execute(interaction) {
    ...
    const embed = new EmbedBuilder()
      .setTitle('◈ Квалификационный тест — mUHC')
      ...
      .setFooter({ text: 'Система квалификации mUHC' })
      .setTimestamp()
      .setColor(0xe67e22);

    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('btn_qualtest_muhc').setLabel('Пройти тест mUHC').setStyle(ButtonStyle.Primary).setEmoji('📋'),
    );

    await interaction.channel.send({ embeds: [embed], components: [row] });
    await interaction.editReply({ content: '✅ Панель квалификации mUHC отправлена!' });
  },

  async handleButton(interaction) {
    if (interaction.customId === 'btn_qualtest_muhc') {
      const modal = new ModalBuilder().setCustomId('qualtest_muhc_modal').setTitle('Квалификационный тест mUHC');
      modal.addComponents(
  ...
  async handleModal(interaction) {
    if (interaction.customId !== 'qualtest_muhc_modal') return;
    ...
    const embed = new EmbedBuilder()
      .setTitle('📋 Заявка на квалификационный тест mUHC')
      .addFields(
    ...
    await ticketChannel.send({ content: `<@&${MAIN_PING_ROLE}> | Новая заявка на тест mUHC от ${user}`, embeds: [embed], components: [closeRow] });
    await interaction.reply({ content: `✅ Заявка на mUHC отправлена! Канал: ${ticketChannel}`, ephemeral: true });
  },
};
