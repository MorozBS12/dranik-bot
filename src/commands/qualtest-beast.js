...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('qualtest-beast')
    .setDescription('Квалификационный тест (BEAST)'),
  ...
  async execute(interaction) {
    ...
    const embed = new EmbedBuilder()
      .setTitle('◈ Квалификационный тест — BEAST')
      .setDescription('Нажмите кнопку ниже, чтобы пройти квалификационный тест на BEAST.\n\n⏳ КД на тест — **7 дней**.\n⚠️ Предоставление недостоверной информации приведет к отказу.')
      .setFooter({ text: 'Система квалификации BEAST' })
      .setTimestamp()
      .setColor(0xe74c3c);

    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('btn_qualtest_beast').setLabel('Пройти тест BEAST').setStyle(ButtonStyle.Danger).setEmoji('📋'),
    );

    await interaction.channel.send({ embeds: [embed], components: [row] });
    await interaction.editReply({ content: '✅ Панель квалификации BEAST отправлена!' });
  },

  async handleButton(interaction) {
    if (interaction.customId === 'btn_qualtest_beast') {
      const modal = new ModalBuilder().setCustomId('qualtest_beast_modal').setTitle('Квалификационный тест BEAST');
      modal.addComponents(
  ...
  async handleModal(interaction) {
    if (interaction.customId !== 'qualtest_beast_modal') return;
    ...
    const embed = new EmbedBuilder()
      .setTitle('📋 Заявка на квалификационный тест BEAST')
      .addFields(
    ...
    await ticketChannel.send({ content: `<@&${MAIN_PING_ROLE}> | Новая заявка на тест BEAST от ${user}`, embeds: [embed], components: [closeRow] });
    await interaction.reply({ content: `✅ Заявка на BEAST отправлена! Канал: ${ticketChannel}`, ephemeral: true });
  },
};
