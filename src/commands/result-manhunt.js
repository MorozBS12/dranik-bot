...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('result-manhunt')
    .setDescription('Выдать ранг игроку (Manhunt)')
    .addUserOption(opt => opt.setName('member').setDescription('Игрок').setRequired(true)),

  async execute(interaction) {
    const allowedRoles = ['1484266536188514396','1484266955635556483','1486807581388181626','1487138476133449870','1496571465376596089'];
    ...
    const modal = new ModalBuilder().setCustomId(`result-manhunt_${member.id}`).setTitle('Результат тестирования (Manhunt)');
    modal.addComponents(
  ...
  async handleModal(interaction) {
    ...
    try {
      ...
      if (targetMember) {
        const roleToAssign = interaction.guild.roles.cache.find(r => r.name === `Manhunt ${tier}`);
        if (roleToAssign) {
          const oldRoles = targetMember.roles.cache.filter(r => VALID_TIERS.some(t => r.name === `Manhunt ${t}`));
        ...
        } else console.warn(`Роль "Manhunt ${tier}" не найдена!`);
      }
      ...
      const embed = new EmbedBuilder()
        .setColor(0xa0522d)
        .setAuthor({ name: `Результат теста Manhunt ${nickname} 🏆`, iconURL: interaction.guild.iconURL() })
        ...
        .addFields(
          ...
          { name: 'Новый ранг',      value: `Manhunt ${tier}`, inline: false },
        )
        .setTimestamp();

      await interaction.channel.send({ content: `<@${targetUserId}>`, embeds: [embed] });
      await interaction.editReply({ content: `✅ Результат Manhunt отправлен! Роль **Manhunt ${tier}** выдана.` });
      if (targetMember) await targetMember.send({ content: `Вы получили новый тир Manhunt: **${tier}** на сервере ${interaction.guild.name}` }).catch(() => {});
    } catch (error) {
      console.error('Ошибка result-manhunt:', error);
