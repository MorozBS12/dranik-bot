...
const MAIN_PING_ROLE = '1491115817910145134';
const CATEGORY_ID = '1490739915133157640';

module.exports = {
  data: new SlashCommandBuilder()
    .setName('qualtest-diacart')
    .setDescription('Квалификационный тест (DiaCart)'),
  ...
  async execute(interaction) {
    ...
    const embed = new EmbedBuilder()
      .setTitle('◈ Квалификационный тест — DiaCart')
      ...
      .setFooter({ text: 'Система квалификации DiaCart' })
      .setTimestamp()
      .setColor(0xe74c3c);

    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('btn_qualtest_diacart').setLabel('Пройти тест DiaCart').setStyle(ButtonStyle.Danger).setEmoji('📋'),
    );

    await interaction.channel.send({ embeds: [embed], components: [row] });
    await interaction.editReply({ content: '✅ Панель квалификации DiaCart отправлена!' });
  },

  async handleButton(interaction) {
    if (interaction.customId === 'btn_qualtest_diacart') {
      const modal = new ModalBuilder().setCustomId('qualtest_diacart_modal').setTitle('Квалификационный тест DiaCart');
      modal.addComponents(
  ...
  async handleModal(interaction) {
    if (interaction.customId !== 'qualtest_diacart_modal') return;
    ...
    const ticketChannel = await guild.channels.create({
      name: `diacart-тест-${user.username}`,
    ...
    const embed = new EmbedBuilder()
      .setTitle('📋 Заявка на квалификационный тест DiaCart')
      .addFields(
    ...
    await ticketChannel.send({ content: `<@&${MAIN_PING_ROLE}> | Новая заявка на тест DiaCart от ${user}`, embeds: [embed], components: [closeRow] });
    await interaction.reply({ content: `✅ Заявка на DiaCart отправлена! Канал: ${ticketChannel}`, ephemeral: true });
  },
};
