...
const MODER_ROLES = [
    '1484266536188514396','1484266955635556483','1486807581388181626','1487138476133449870','1496571286506311761'
];
const MAIN_PING_ROLE = '1496571286506311761';
const CATEGORY_ID = '1496574503982072001';

module.exports = {
  data: new SlashCommandBuilder()
    .setName('qualtest-nethpot')
    .setDescription('Квалификационный тест (NethPot)'),
  ...
  async execute(interaction) {
    ...
    const embed = new EmbedBuilder()
      .setTitle('◈ Квалификационный тест — NethPot')
      ...
      .setFooter({ text: 'Система квалификации NethPot' })
      .setTimestamp()
      .setColor(0xe74c3c);

    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('btn_qualtest_nethpot').setLabel('Пройти тест NethPot').setStyle(ButtonStyle.Danger).setEmoji('📋'),
    );

    await interaction.channel.send({ embeds: [embed], components: [row] });
    await interaction.editReply({ content: '✅ Панель квалификации NethPot отправлена!' });
  },

  async handleButton(interaction) {
    if (interaction.customId === 'btn_qualtest_nethpot') {
      const modal = new ModalBuilder().setCustomId('qualtest_nethpot_modal').setTitle('Квалификационный тест NethPot');
      modal.addComponents(
  ...
  async handleModal(interaction) {
    if (interaction.customId !== 'qualtest_nethpot_modal') return;
    ...
    const ticketChannel = await guild.channels.create({
      name: `nethpot-тест-${user.username}`,
    ...
    const embed = new EmbedBuilder()
      .setTitle('📋 Заявка на квалификационный тест NethPot')
      .addFields(
    ...
    await ticketChannel.send({ content: `<@&${MAIN_PING_ROLE}> | Новая заявка на тест NethPot от ${user}`, embeds: [embed], components: [closeRow] });
    await interaction.reply({ content: `✅ Заявка на NethPot отправлена! Канал: ${ticketChannel}`, ephemeral: true });
  },
};
