...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('result-dsmp')
    .setDescription('Выдать ранг игроку (DSMP)')
    .addUserOption(opt => opt.setName('member').setDescription('Игрок').setRequired(true)),

  async execute(interaction) {
    const allowedRoles = ['1481040977102897366','1480616685999226971','1481041265469423667','1491115960877187134','1401145264571682886'];
    ...
    const modal = new ModalBuilder().setCustomId(`result-dsmp_${member.id}`).setTitle('Результат тестирования (DSMP)');
    modal.addComponents(
  ...
  async handleModal(interaction) {
    ...
    try {
      ...
      if (targetMember) {
        const roleToAssign = interaction.guild.roles.cache.find(r => r.name === `DSMP ${tier}`);
        if (roleToAssign) {
          const oldRoles = targetMember.roles.cache.filter(r => VALID_TIERS.some(t => r.name === `DSMP ${t}`));
        ...
        } else console.warn(`Роль "DSMP ${tier}" не найдена!`);
      }
      ...
      const embed = new EmbedBuilder()
        .setColor(0x9b59b6)
        .setAuthor({ name: `Результат теста DSMP ${nickname} 🏆`, iconURL: interaction.guild.iconURL() })
        ...
        .addFields(
          ...
          { name: 'Новый ранг',      value: `DSMP ${tier}`,   inline: false },
        )
        .setTimestamp();

      await interaction.channel.send({ content: `<@${targetUserId}>`, embeds: [embed] });
      await interaction.editReply({ content: `✅ Результат DSMP отправлен! Роль **DSMP ${tier}** выдана.` });
      if (targetMember) await targetMember.send({ content: `Вы получили новый тир DSMP: **${tier}** на сервере ${interaction.guild.name}` }).catch(() => {});
    } catch (error) {
      console.error('Ошибка result-dsmp:', error);
