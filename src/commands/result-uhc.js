...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('result-uhc')
    .setDescription('Выдать ранг игроку (UHC)')
  ...
  async execute(interaction) {
    ...
    const modal = new ModalBuilder().setCustomId(`result-uhc_${member.id}`).setTitle('Результат тестирования (UHC)');
    modal.addComponents(
  ...
  async handleModal(interaction) {
    ...
    try {
      ...
      if (targetMember) {
        const roleToAssign = interaction.guild.roles.cache.find(r => r.name === `UHC ${tier}`);
        if (roleToAssign) {
          const oldTierRoles = targetMember.roles.cache.filter(r => VALID_TIERS.some(t => r.name === `UHC ${t}`));
          ...
          console.warn(`Роль "UHC ${tier}" не найдена на сервере!`);
        }
      }
      ...
      const embed = new EmbedBuilder()
        .setColor(0xe67e22)
        .setAuthor({ name: `Результат теста UHC ${nickname} 🏆`, iconURL: interaction.guild.iconURL() })
        ...
        .addFields(
          ...
          { name: 'Новый ранг',      value: `UHC ${tier}`, inline: false },
        )
        .setTimestamp();

      await interaction.channel.send({ content: `<@${targetUserId}>`, embeds: [embed] });
      await interaction.editReply({ content: `✅ Результат UHC отправлен! Роль **UHC ${tier}** выдана.` });
      if (targetMember) await targetMember.send({ content: `Вы получили новый тир UHC: **${tier}** на сервере ${interaction.guild.name}` }).catch(() => {});
    } catch (error) {
      console.error('Ошибка result-uhc:', error);
