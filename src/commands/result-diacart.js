...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('result-diacart')
    .setDescription('Выдать ранг игроку (DiaCart)')
    .addUserOption(opt => opt.setName('member').setDescription('Игрок').setRequired(true)),

  async execute(interaction) {
    const allowedRoles = ['1481040977102897366','1480616685999226971','1481041265469423667','1401145264571682886','1491115817910145134','1401145264571682886'];
    ...
    const modal = new ModalBuilder().setCustomId(`result-diacart_${member.id}`).setTitle('Результат тестирования (DiaCart)');
    modal.addComponents(
  ...
  async handleModal(interaction) {
    ...
    try {
      ...
      if (targetMember) {
        const roleToAssign = interaction.guild.roles.cache.find(r => r.name === `DiaCart ${tier}`);
        if (roleToAssign) {
          const oldRoles = targetMember.roles.cache.filter(r => VALID_TIERS.some(t => r.name === `DiaCart ${t}`));
        ...
        } else console.warn(`Роль "DiaCart ${tier}" не найдена!`);
      }
      ...
      const embed = new EmbedBuilder()
        .setColor(0xe74c3c)
        .setAuthor({ name: `Результат теста DiaCart ${nickname} 🏆`, iconURL: interaction.guild.iconURL() })
        ...
        .addFields(
          ...
          { name: 'Новый ранг',      value: `DiaCart ${tier}`,   inline: false },
        )
        .setTimestamp();

      await interaction.channel.send({ content: `<@${targetUserId}>`, embeds: [embed] });
      await interaction.editReply({ content: `✅ Результат DiaCart отправлен! Роль **DiaCart ${tier}** выдана.` });
      if (targetMember) await targetMember.send({ content: `Вы получили новый тир DiaCart: **${tier}** на сервере ${interaction.guild.name}` }).catch(() => {});
    } catch (error) {
      console.error('Ошибка result-diacart:', error);
