...
async function generateResultCard({ nickname, oldTier, newTier, testerName, testerAvatarUrl, guildName, guildIconUrl, skinUrl }) {
  ...
  ctx.font = '500 16px Liberation Sans';
  ...
  if (guildImg) {
    ...
    ctx.clip();
    ctx.drawImage(guildImg, 274, 132, 36, 36);
    ctx.restore();
  } else {
    ctx.fillStyle = KIT_COLOR;
  ...
  ctx.fillText(guildName || 'ATiers', 320, 148);
  ...
  // --- RIGHT: previous tier ---
  ctx.fillStyle = 'rgba(255,255,255,0.5)';
  ctx.font = '500 14px Liberation Sans';
  ctx.fillText('Предыдущий тир', 530, 80);

  ctx.fillStyle = '#ffffff';
  ctx.font = 'bold 26px Liberation Sans';
  ...
  ctx.fillStyle = '#ffffff';
  ctx.font = 'bold 72px Liberation Sans';
  ctx.fillText(newTier, 530, 250);

  ctx.fillStyle = KIT_COLOR;
  ctx.font = 'bold 28px Liberation Sans';
