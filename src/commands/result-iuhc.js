...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('result-iuhc')
    .setDescription('Выдать ранг игроку (iUHC)')
    .addUserOption(opt => opt.setName('member').setDescription('Игрок').setRequired(true)),

  async execute(interaction) {
    const allowedRoles = ['1484266536188514396','1484266955635556483','1486807581388181626','1487138476133449870','1486844213994717266','1496577008455520389'];
    ...
    const modal = new ModalBuilder().setCustomId(`result-iuhc_${member.id}`).setTitle('Результат тестирования (iUHC)');
    modal.addComponents(
  ...
  async handleModal(interaction) {
    ...
    try {
      ...
      if (targetMember) {
        const roleToAssign = interaction.guild.roles.cache.find(r => r.name === `iUHC ${tier}`);
        if (roleToAssign) {
          const oldRoles = targetMember.roles.cache.filter(r => VALID_TIERS.some(t => r.name === `iUHC ${t}`));
        ...
        } else console.warn(`Роль "iUHC ${tier}" не найдена!`);
      }
      ...
      const embed = new EmbedBuilder()
        .setColor(0xf1c40f)
        .setAuthor({ name: `Результат теста iUHC ${nickname} 🏆`, iconURL: interaction.guild.iconURL() })
        ...
        .addFields(
          ...
          { name: 'Новый ранг',      value: `iUHC ${tier}`, inline: false },
        )
        .setTimestamp();

      await interaction.channel.send({ content: `<@${targetUserId}>`, embeds: [embed] });
      await interaction.editReply({ content: `✅ Результат iUHC отправлен! Роль **iUHC ${tier}** выдана.` });
      if (targetMember) await targetMember.send({ content: `Вы получили новый тир iUHC: **${tier}** на сервере ${interaction.guild.name}` }).catch(() => {});
    } catch (error) {
      console.error('Ошибка result-iuhc:', error);
