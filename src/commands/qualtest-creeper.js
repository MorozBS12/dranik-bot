...
const MODER_ROLES = [
  ...
  '1491115725476335768','1480616685999226971','1487138476133449870'
];
const MAIN_PING_ROLE = '1491115725476335768';
const CATEGORY_ID = '1490739834665566319';

module.exports = {
  data: new SlashCommandBuilder()
    .setName('qualtest-creeper')
    .setDescription('Квалификационный тест (Creeper)'),
  ...
  async execute(interaction) {
    ...
    const embed = new EmbedBuilder()
      .setTitle('◈ Квалификационный тест — Creeper')
      ...
      .setFooter({ text: 'Система квалификации Creeper' })
      .setTimestamp()
      .setColor(0x2ecc71);

    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('btn_qualtest_creeper').setLabel('Пройти тест Creeper').setStyle(ButtonStyle.Success).setEmoji('📋'),
    );

    await interaction.channel.send({ embeds: [embed], components: [row] });
    await interaction.editReply({ content: '✅ Панель квалификации Creeper отправлена!' });
  },

  async handleButton(interaction) {
    if (interaction.customId === 'btn_qualtest_creeper') {
      const modal = new ModalBuilder().setCustomId('qualtest_creeper_modal').setTitle('Квалификационный тест Creeper');
      modal.addComponents(
  ...
  async handleModal(interaction) {
    if (interaction.customId !== 'qualtest_creeper_modal') return;
    ...
    const ticketChannel = await guild.channels.create({
      name: `creeper-тест-${user.username}`,
    ...
    const embed = new EmbedBuilder()
      .setTitle('📋 Заявка на квалификационный тест Creeper')
      .addFields(
    ...
    await ticketChannel.send({ content: `<@&${MAIN_PING_ROLE}> | Новая заявка на тест Creeper от ${user}`, embeds: [embed], components: [closeRow] });
    await interaction.reply({ content: `✅ Заявка на Creeper отправлена! Канал: ${ticketChannel}`, ephemeral: true });
  },
};
