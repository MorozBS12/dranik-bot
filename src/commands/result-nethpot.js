...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('result-nethpot')
    .setDescription('Выдать ранг игроку (NethPot)')
    .addUserOption(opt => opt.setName('member').setDescription('Игрок').setRequired(true)),

  async execute(interaction) {
    const allowedRoles = ['1484266536188514396','1484266955635556483','1486807581388181626','1487138476133449870','1496571286506311761'];
    ...
    const modal = new ModalBuilder().setCustomId(`result-nethpot_${member.id}`).setTitle('Результат тестирования (NethPot)');
    modal.addComponents(
  ...
  async handleModal(interaction) {
    ...
    try {
      ...
      if (targetMember) {
        const roleToAssign = interaction.guild.roles.cache.find(r => r.name === `NethPot ${tier}`);
        if (roleToAssign) {
          const oldRoles = targetMember.roles.cache.filter(r => VALID_TIERS.some(t => r.name === `NethPot ${t}`));
        ...
        } else console.warn(`Роль "NethPot ${tier}" не найдена!`);
      }
      ...
      const embed = new EmbedBuilder()
        .setColor(0xe74c3c)
        .setAuthor({ name: `Результат теста NethPot ${nickname} 🏆`, iconURL: interaction.guild.iconURL() })
        ...
        .addFields(
          ...
          { name: 'Новый ранг',      value: `NethPot ${tier}`, inline: false },
        )
        .setTimestamp();

      await interaction.channel.send({ content: `<@${targetUserId}>`, embeds: [embed] });
      await interaction.editReply({ content: `✅ Результат NethPot отправлен! Роль **NethPot ${tier}** выдана.` });
      if (targetMember) await targetMember.send({ content: `Вы получили новый тир NethPot: **${tier}** на сервере ${interaction.guild.name}` }).catch(() => {});
    } catch (error) {
      console.error('Ошибка result-nethpot:', error);
