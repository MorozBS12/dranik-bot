...
const MODER_ROLES = [
  ...
  '1491115960877187134','1480616685999226971','1487138476133449870'
];
const MAIN_PING_ROLE = '1491115960877187134';
const CATEGORY_ID = '1490739956102988007';

module.exports = {
  data: new SlashCommandBuilder()
    .setName('qualtest-dsmp')
    .setDescription('Квалификационный тест (DSMP)'),
  ...
  async execute(interaction) {
    ...
    const embed = new EmbedBuilder()
      .setTitle('◈ Квалификационный тест — DSMP')
      ...
      .setFooter({ text: 'Система квалификации DSMP' })
      .setTimestamp()
      .setColor(0x9b59b6);

    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('btn_qualtest_dsmp').setLabel('Пройти тест DSMP').setStyle(ButtonStyle.Primary).setEmoji('📋'),
    );

    await interaction.channel.send({ embeds: [embed], components: [row] });
    await interaction.editReply({ content: '✅ Панель квалификации DSMP отправлена!' });
  },

  async handleButton(interaction) {
    if (interaction.customId === 'btn_qualtest_dsmp') {
      const modal = new ModalBuilder().setCustomId('qualtest_dsmp_modal').setTitle('Квалификационный тест DSMP');
      modal.addComponents(
  ...
  async handleModal(interaction) {
    if (interaction.customId !== 'qualtest_dsmp_modal') return;
    ...
    const embed = new EmbedBuilder()
      .setTitle('📋 Заявка на квалификационный тест DSMP')
      .addFields(
    ...
    await ticketChannel.send({ content: `<@&${MAIN_PING_ROLE}> | Новая заявка на тест DSMP от ${user}`, embeds: [embed], components: [closeRow] });
    await interaction.reply({ content: `✅ Заявка на DSMP отправлена! Канал: ${ticketChannel}`, ephemeral: true });
  },
};
