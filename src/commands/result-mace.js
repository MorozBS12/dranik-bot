...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('result-mace')
    .setDescription('Выдать ранг игроку (DiaMace)')
    .addUserOption(opt => opt.setName('member').setDescription('Игрок').setRequired(true)),

  async execute(interaction) {
    const allowedRoles = ['1481040977102897366','1491115895043657869','1480616685999226971','1481041265469423667','1401145264571682886'];
    ...
    const modal = new ModalBuilder().setCustomId(`result-mace_${member.id}`).setTitle('Результат тестирования (DiaMace)');
    modal.addComponents(
  ...
  async handleModal(interaction) {
    ...
    try {
      ...
      if (targetMember) {
        const roleToAssign = interaction.guild.roles.cache.find(r => r.name === `DiaMace ${tier}`);
        if (roleToAssign) {
          const oldRoles = targetMember.roles.cache.filter(r => VALID_TIERS.some(t => r.name === `DiaMace ${t}`));
        ...
        } else console.warn(`Роль "DiaMace ${tier}" не найдена!`);
      }
      ...
      const embed = new EmbedBuilder()
        .setColor(0x3498db)
        .setAuthor({ name: `Результат теста DiaMace ${nickname} 🏆`, iconURL: interaction.guild.iconURL() })
        ...
        .addFields(
          ...
          { name: 'Новый ранг',      value: `DiaMace ${tier}`,   inline: false },
        )
        .setTimestamp();

      await interaction.channel.send({ content: `<@${targetUserId}>`, embeds: [embed] });
      await interaction.editReply({ content: `✅ Результат DiaMace отправлен! Роль **DiaMace ${tier}** выдана.` });
      if (targetMember) await targetMember.send({ content: `Вы получили новый тир DiaMace: **${tier}** на сервере ${interaction.guild.name}` }).catch(() => {});
    } catch (error) {
      console.error('Ошибка result-mace:', error);
