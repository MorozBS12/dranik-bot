...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('result-speed')
    .setDescription('Выдать ранг игроку (Speed)')
    .addUserOption(opt => opt.setName('member').setDescription('Игрок').setRequired(true)),

  async execute(interaction) {
    const allowedRoles = ['1481040977102897366','1491116148379353298','1480616685999226971','1481041265469423667','1401145264571682886'];
    ...
    const modal = new ModalBuilder().setCustomId(`result-speed_${member.id}`).setTitle('Результат тестирования (Speed)');
    modal.addComponents(
  ...
  async handleModal(interaction) {
    ...
    try {
      ...
      if (targetMember) {
        const roleToAssign = interaction.guild.roles.cache.find(r => r.name === `Speed ${tier}`);
        if (roleToAssign) {
          const oldRoles = targetMember.roles.cache.filter(r => VALID_TIERS.some(t => r.name === `Speed ${t}`));
        ...
        } else console.warn(`Роль "Speed ${tier}" не найдена!`);
      }
      ...
      const embed = new EmbedBuilder()
        .setColor(0x3498db)
        .setAuthor({ name: `Результат теста Speed ${nickname} 🏆`, iconURL: interaction.guild.iconURL() })
        ...
        .addFields(
          ...
          { name: 'Новый ранг',      value: `Speed ${tier}`,   inline: false },
        )
        .setTimestamp();

      await interaction.channel.send({ content: `<@${targetUserId}>`, embeds: [embed] });
      await interaction.editReply({ content: `✅ Результат Speed отправлен! Роль **Speed ${tier}** выдана.` });
      if (targetMember) await targetMember.send({ content: `Вы получили новый тир Speed: **${tier}** на сервере ${interaction.guild.name}` }).catch(() => {});
    } catch (error) {
      console.error('Ошибка result-speed:', error);
