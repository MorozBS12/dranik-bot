...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('result-bed')
    .setDescription('Выдать ранг игроку (Bed)')
    .addUserOption(opt => opt.setName('member').setDescription('Игрок').setRequired(true)),

  async execute(interaction) {
    const allowedRoles = ['1484266536188514396','1484266955635556483','1486807581388181626','1487138476133449870','1496571411467210808'];
    ...
    const modal = new ModalBuilder().setCustomId(`result-bed_${member.id}`).setTitle('Результат тестирования (Bed)');
    modal.addComponents(
  ...
  async handleModal(interaction) {
    ...
    try {
      ...
      if (targetMember) {
        const roleToAssign = interaction.guild.roles.cache.find(r => r.name === `Bed ${tier}`);
        if (roleToAssign) {
          const oldRoles = targetMember.roles.cache.filter(r => VALID_TIERS.some(t => r.name === `Bed ${t}`));
        ...
        } else console.warn(`Роль "Bed ${tier}" не найдена!`);
      }
      ...
      const embed = new EmbedBuilder()
        .setColor(0xe74c3c)
        .setAuthor({ name: `Результат теста Bed ${nickname} 🏆`, iconURL: interaction.guild.iconURL() })
        ...
        .addFields(
          ...
          { name: 'Новый ранг',      value: `Bed ${tier}`, inline: false },
        )
        .setTimestamp();

      await interaction.channel.send({ content: `<@${targetUserId}>`, embeds: [embed] });
      await interaction.editReply({ content: `✅ Результат Bed отправлен! Роль **Bed ${tier}** выдана.` });
      if (targetMember) await targetMember.send({ content: `Вы получили новый тир Bed: **${tier}** на сервере ${interaction.guild.name}` }).catch(() => {});
    } catch (error) {
      console.error('Ошибка result-bed:', error);
