...
const CATEGORY_ID = '1490739915133157640';

module.exports = {
  data: new SlashCommandBuilder()
    .setName('qualtest-dcart')
    .setDescription('Квалификационный тест (Dcart)'),
  ...
  async execute(interaction) {
    ...
    const embed = new EmbedBuilder()
      .setTitle('◈ Квалификационный тест — Dcart')
      .setDescription('Нажмите кнопку ниже, чтобы пройти квалификационный тест на Dcart.\n\n⏳ КД на тест — **7 дней**.\n⚠️ Предоставление недостоверной информации приведет к отказу.')
      .setFooter({ text: 'Система квалификации Dcart' })
      .setTimestamp()
      .setColor(0xe74c3c);

    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('btn_qualtest_dcart').setLabel('Пройти тест Dcart').setStyle(ButtonStyle.Danger).setEmoji('📋'),
    );

    await interaction.channel.send({ embeds: [embed], components: [row] });
    await interaction.editReply({ content: '✅ Панель квалификации Dcart отправлена!' });
  },

  async handleButton(interaction) {
    if (interaction.customId === 'btn_qualtest_dcart') {
      const modal = new ModalBuilder().setCustomId('qualtest_dcart_modal').setTitle('Квалификационный тест Dcart');
      modal.addComponents(
  ...
  async handleModal(interaction) {
    if (interaction.customId !== 'qualtest_dcart_modal') return;
    ...
    const embed = new EmbedBuilder()
      .setTitle('📋 Заявка на квалификационный тест Dcart')
      .addFields(
    ...
    await ticketChannel.send({ content: `<@&${MAIN_PING_ROLE}> | Новая заявка на тест Dcart от ${user}`, embeds: [embed], components: [closeRow] });
    await interaction.reply({ content: `✅ Заявка на Dcart отправлена! Канал: ${ticketChannel}`, ephemeral: true });
  },
};
