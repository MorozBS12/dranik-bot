...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('result-low')
    .setDescription('Выдать ранг игроку (LT)')
  ...
  async execute(interaction) {
    ...
    if (!hasPermission) return interaction.reply({ content: '❌ У вас нет прав для использования этой команды!', ephemeral: true });

    const member = interaction.options.getUser('member');
    const modal = new ModalBuilder().setCustomId(`result-low_${member.id}`).setTitle('Результат тестирования');
    modal.addComponents(
  ...
  async handleModal(interaction) {
    ...
    try {
      ...
      if (targetMember) {
        const roleToAssign = interaction.guild.roles.cache.find(r => r.name === `Mace ${tier}`);
        if (roleToAssign) {
          const oldTierRoles = targetMember.roles.cache.filter(r => VALID_TIERS.some(t => r.name === `Mace ${t}`));
          ...
          console.warn(`Роль "Mace ${tier}" не найдена на сервере!`);
        }
      }
      ...
      const embed = new EmbedBuilder()
        .setColor(0x3498db)
        .setAuthor({ name: `Результат теста ${nickname} 🏆`, iconURL: interaction.guild.iconURL() })
        ...
        .addFields(
          ...
          { name: 'Новый ранг',      value: `Mace ${tier}`,  inline: false },
        )
        .setTimestamp();

      await interaction.channel.send({ content: `<@${targetUserId}>`, embeds: [embed] });
      await interaction.editReply({ content: `✅ Результат отправлен! Роль **Mace ${tier}** выдана.` });
      if (targetMember) await targetMember.send({ content: `Вы получили новый тир: **Mace ${tier}** на сервере ${interaction.guild.name}` }).catch(() => {});
    } catch (error) {
      console.error('Ошибка result-low:', error);
