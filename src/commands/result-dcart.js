...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('result-dcart')
    .setDescription('Выдать ранг игроку (Dcart)')
  ...
  async execute(interaction) {
    ...
    const modal = new ModalBuilder().setCustomId(`result-dcart_${member.id}`).setTitle('Результат тестирования (Dcart)');
    modal.addComponents(
  ...
  async handleModal(interaction) {
    ...
    try {
      ...
      if (targetMember) {
        const roleToAssign = interaction.guild.roles.cache.find(r => r.name === `Dcart ${tier}`);
        if (roleToAssign) {
          const oldTierRoles = targetMember.roles.cache.filter(r => VALID_TIERS.some(t => r.name === `Dcart ${t}`));
        ...
        } else console.warn(`Роль "Dcart ${tier}" не найдена!`);
      }
      ...
      const embed = new EmbedBuilder()
        .setColor(0xe74c3c)
        .setAuthor({ name: `Результат теста Dcart ${nickname} 🏆`, iconURL: interaction.guild.iconURL() })
        ...
        .addFields(
          ...
          { name: 'Новый ранг',      value: `Dcart ${tier}`,  inline: false },
        )
        .setTimestamp();

      await interaction.channel.send({ content: `<@${targetUserId}>`, embeds: [embed] });
      await interaction.editReply({ content: `✅ Результат Dcart отправлен! Роль **Dcart ${tier}** выдана.` });
      if (targetMember) await targetMember.send({ content: `Вы получили новый тир Dcart: **${tier}** на сервере ${interaction.guild.name}` }).catch(() => {});
    } catch (error) {
      console.error('Ошибка result-dcart:', error);
